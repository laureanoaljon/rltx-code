...
class Variety extends CI_Model {
    
    function get_var_top($location_code, $location_type, $year, $ecosystem){
        $this->db->select('l.locName AS location_name, p.year AS year, p.rank AS rank, p.variety AS label, p.percent AS value');
        $this->db->from('rb_variety p');
    ...
    function get_var_top_prri($location_code, $location_type, $year, $ecosystem){
        $this->db->select('l.locName AS location_name, p.year AS year, p.rank AS rank, p.variety AS label, p.percent AS value');
        $this->db->from('rb_varietyprri p');
        $this->db->join('ids_location l', 'p.locCode = l.locCode AND p.locType = l.locType');
        $this->db->join('ids_ecosystem e', 'p.eco = e.eco');
        $this->db->where('p.locCode', $location_code);
        $this->db->where('p.locType', $location_type);
    ...
    function get_var_top_prov($location_code = NULL, $location_type, $year_start, $ecosystem, $year_end = NULL, $limit = NULL){
        ...
        $this->db->select('p.variety AS label');
